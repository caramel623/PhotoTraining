...
> 2026-09-14 交付：v0.0.9 EXE 已編譯，218 項回歸、正式 EXE 空白工作區啟動及兩次離線安裝器皆通過。下列「尚未編譯」為本次編譯前紀錄；以 HANDOFF_2026-09-14_REVIEW.md 為準。

> 2026-09-14：點車輛縮圖可放大預覽，點圖片外空白／Esc 關閉，預覽 Enter 不確認群組；Ctrl／Shift 保留多選。來源為目前顯示圖片（優先車輛裁切，否則原圖），依比例放大。此功能尚未重編 EXE。

> 2026-09-14：人工檢核 Enter／數字鍵盤 Enter 改為直接確認整組，依目前清單前往下一個未確認群組，尾端回繞、略過已確認與 metadata 佇列。無剩餘群組時只在頁面提示；停用 Enter 長按連發。檢核專項 23 項通過；此修改尚未重編 EXE 或提交。
